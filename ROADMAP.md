...
| 5 | Controller — OCI pipeline | Can the controller poll Zot, cosign-verify, and write the ConfigBundle CR reliably? | — | Not started | Puller as `ctrl.Runnable` (not a reconciler); apply ConfigBundle CR spec via SSA **without** ForceOwnership (local overrides preserved); status subresource: ArtifactFetched + SignatureVerified conditions, lastAppliedDigest, lastAppliedAt; idempotent on digest. **Open design question:** SSA has no partial apply — a single conflict (locally-owned field) causes the entire apply to fail. Puller must decide: inspect managedFields first and omit locally-owned fields from the patch, OR use ForceOwnership (wipes overrides). Must be resolved before implementation. **Prerequisite:** add `+listType=map +listMapKey=serviceTag` to `servers[]` in `ConfigBundleSpec` before implementing the Puller. Without it, SSA treats `servers[]` as atomic — a local admin overriding one field in one server entry takes ownership of the ENTIRE array. Combined with no-partial-apply, the Puller's next apply fails on the whole array, blocking ALL server config changes for the datacenter (not just the overridden entry). With the annotation, each server entry is independently trackable and the Puller can omit only the specific locally-owned fields while still updating uncontested servers. |
